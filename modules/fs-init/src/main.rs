...
fn pack_up_user_applications() {
    ...
    println!(
        "src_path = {}\ntarget_path = {}\noutput_file = {}",
        src_path, target_path, output_file
    );
    ...
    create_new_fs(output_file).unwrap();
...
fn create_new_fs(name: &str) -> io::Result<()> {
    let img_file = fs::OpenOptions::new()
        ...
        .open(&name)
        .unwrap();
    img_file.set_len(32 * 2048 * 512).unwrap();
...
fn traverse_fat_dir<'a>(
    ...
    println!(
        "{:4}  {}  {}",
        file_size_to_str(file.len()),
        date_time_to_str(file.modified()),
        file.file_name()
    );
    // 如果是子目录，则再继续遍历
    if file.is_dir() {
