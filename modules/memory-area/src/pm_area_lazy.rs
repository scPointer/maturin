...
use super::defs::{
    addr_to_page_id, align_down, align_up, page_count, phys_to_virt, PTEFlags, PAGE_SIZE,
};
use super::{MemoryAreaConfig, MemoryAreaConfigDefault, PageFrameInter, PmArea, VmArea};

#[cfg(not(feature = "backend"))]
type BackEndFile = ();

/// lazy 分配的物理地址段。当 page fault 发生时会由 VmArea 负责调用这段 PmAreaLazy 进行实际分配
pub struct PmAreaLazy<Frame: PageFrameInter, Config: MemoryAreaConfig = MemoryAreaConfigDefault> {
    frames: Vec<Option<Frame>>,
    backend: Option<BackEndFile>,
}

impl<Frame, Config> PmArea for PmAreaLazy<Frame, Config> {
    fn size(&self) -> usize {
...
impl<Frame, Config> PmAreaLazy<Frame, Config> {
    /// 生成新的pma，给定空间大小但不立即分配物理页
    pub fn new(page_count: usize, backend: Option<BackEndFile>) -> Result<Self> {
        if page_count == 0 {
            error!("page_count is 0 in PmAreaLazy");
            return Err(OSError::PmArea_InvalidRange);
        }
        if page_count > page_count(USER_VIRT_ADDR_LIMIT) {
