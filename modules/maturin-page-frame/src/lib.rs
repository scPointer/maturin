...
/// Config 的默认实现，可以直接使用，也可自己实现 `PageFrameConfig`
pub struct PageFrameConfigDefault;
impl defs::PageFrameConfig for PageFrameConfigDefault {}
type Allocator<Config> = allocator::FrameAllocatorWrapper<Config>;

/// 页帧定义，自动用 new 和 Drop 包装了页帧的分配和回收过程
#[derive(Debug)]
pub struct Frame<Config: PageFrameConfig = PageFrameConfigDefault> {
    start_paddr: usize,
    frame_count: usize,
