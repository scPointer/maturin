...
use crate::timer::{TimeVal, get_time_ms};
...
pub struct TimeStat {
    ...
    /// 设置下一次触发计时器的区间
    /// 
    /// 当 timer_remained_ms 归零时，**如果 timer_interval_ms 非零 **，则将其重置为 timer_interval_ms 的值；
    /// 否则，则这个计时器不再触发
    timer_interval_ms: usize,
    /// 当前计时器还剩下多少时间。
    /// 
    /// 根据 timer_type 的规则不断减少，当归零时触发信号
    timer_remained_ms: usize,
}
...
impl TimeStat {
    ...
    pub fn switch_into_task(&mut self) {
        self.kernel_tick = get_time_ms();
    }
    /// 统计时间：(内核态)切出进入当前任务
    pub fn switch_out_task(&mut self) {
        let delta = get_time_ms() - self.kernel_tick;
        self.stime_ms += delta;
        if self.timer_type == TimerType::REAL || self.timer_type == TimerType::PROF {
            self.update_timer_and_send_signal(delta);
        }
    ...
    pub fn output(&self, utime: &mut TimeVal, stime: &mut TimeVal) {
        *utime = self.utime_ms.into();
        *stime = self.stime_ms.into();
    }
    /// 输出毫秒形式的时间统计，用于调试
    pub fn output_raw(&self) -> (usize, usize) {
        (self.utime_ms, self.stime_ms)
    }

    /// 以 TimeVal 形式输出计时器信息
    pub fn get_timer(&self, itimer: &mut ITimerVal) {
        itimer.it_interval = self.timer_interval_ms.into();
        itimer.it_value = self.timer_remained_ms.into();
    }
    ...
    pub fn set_timer(&mut self, itimer: &ITimerVal, timer_type: usize) -> bool {
        ...
        self.timer_interval_ms = itimer.it_interval.into();
        self.timer_remained_ms = itimer.it_value.into();
        self.timer_type != TimerType::NONE
    }
    /// 从计时器中尝试减少一段时间，如果时间归零，则发送信号
    /// (**内部需要获取对应线程的 SignalReceivers，注意死锁，注意保证发送的线程仍存在**)。
    /// 然后根据 timer_interval_ms 更新寄存器
    /// 
    /// 返回是否触发信号
    pub fn update_timer_and_send_signal(&mut self, delta: usize) -> bool {
        if self.timer_remained_ms == 0 { // 等于0说明没有计时器，或者 one-shot 计时器已结束
            return false;
        }
        if self.timer_remained_ms > delta { // 时辰未到，减少寄存器计数
            self.timer_remained_ms -= delta;
            return false;
        }
        // 到此说明计时器已经到时间了，更新计时器
        // 如果是 one-shot 计时器，则 timer_interval_ms == 0，这样赋值也恰好是符合语义的
        self.timer_remained_ms = self.timer_interval_ms;
