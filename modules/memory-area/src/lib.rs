...
pub use pm_area::PmArea;
pub use pm_area_fixed::PmAreaFixed;
pub use pm_area_lazy::PmAreaLazy;
use range_action_map::{ArgsType as PageTableRoot, IdentType as Flags, Segment};

pub struct MemoryAreaConfigDefault;
impl defs::MemoryAreaConfig for MemoryAreaConfigDefault {}

/// 一段访问权限相同的虚拟地址
#[derive(Debug)]
pub struct VmArea<Frame: PageFrameInter, Config: MemoryAreaConfig = MemoryAreaConfigDefault> {
    /// 地址段开头，需要对其页
    pub start: usize,
    /// 地址段结尾，需要对其页
    pub end: usize,
...
impl<Frame: PageFrameInter, Config: MemoryAreaConfig> VmArea<Frame, Config> {
    ...
    fn modify_area_flags(&self, pt: &mut PageTable) -> Result {
        let mut pma = self.pma.lock();
    ...
    pub fn map_area(&self, pt: &mut PageTable) -> Result {
        let mut pma = self.pma.lock();
        for vaddr in (self.start..self.end).step_by(Config::get_page_size()) {
    ...
    /// 删除部分虚拟地址映射
    fn unmap_area_partial(&self, pt: &mut PageTable, start: usize, end: usize) -> Result {
        let mut pma = self.pma.lock();
        for vaddr in (start..end).step_by(Config::get_page_size()) {
    ...
    fn unmap_area(&self, pt: &mut PageTable) -> Result {
    ...
    pub fn copy_to_new_area_empty(&self) -> Result<VmArea> {
        Ok(VmArea {
            start: self.start,
            end: self.end,
            flags: self.flags,
    ...
    pub fn copy_to_new_area_with_data(&self) -> Result<VmArea> {
    ...
    pub fn manually_alloc_page(&self, offset: usize, pt: &mut PageTable) -> Result {
        let mut pma = self.pma.lock();
        let offset = Config::align_down(offset);
        let vaddr = self.start + offset;
        let paddr = pma
...
impl<Frame, Config> Segment for VmArea<Frame, Config> {
